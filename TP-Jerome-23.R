...
plot.PCA(res, habillage=17, cex=0.8)
...
T<- read.csv("http://factominer.free.fr/book/temperature.csv",header=TRUE, sep=";", dec=".", row.names=1)

#2. Affichez ce tableau de donnees a l'ecran
TT <- t(T)

plot(as.numeric(TT[c(1:12),34])*2, type = 'b', col = "white")
for (i in (1:35)) {lines(t(T)[c(1:12),i], type = 'b', col = i)}

print(T$January)
print(T)
View(T)
rownindiv <-rownames (T)
...
class(T)
dim(MAT)
# retirer les 5 dernière colonne de MAT
MAT<-MAT[, -c(13:17)]
# convertir en numérique
MAT<-as.numeric(MAT)

#retier de T les 5 dernier colonnes
T<-T[, -c(13:17)]
# convertir en numéric
T<-apply(X = T, FUN = "as.numeric", 2)

#4. Pour centrer un tableau, le cours donne la formule suivante 
# Y = X- 1_n g'
TMOY <- apply(X=T, FUN = "mean", 2)
TMOY
TTMOY <-colMeans(T)
TTMOY
dim(T)
...
plot(Q1, Q2)
text (x=Q1, y= Q2, labels = rownames(INDIV), pos = 1 , cex = 0.5)
...
plot (CTR1, CTR2)
text (x=CTR1, y= CTR2, labels = rownames(INDIV), pos = 1 , cex = 0.5)
...
#7.NORMALE  les vecteurs propres associes aux axes factoriels de l'ACP.
# combinaison linéaire des 12 variables 
MU1<-val_propre$vectors[,c(1)]
MU1
t(MU1)%*%MU1 # normé ! ouf
...
#8.NORMALE La composante principale associee a l'axe ua est donnee par
#   PHIa = Y.MUa:
# Creez une matrice P dont les deux premieres colonnes 
# sont les deux premieres composantes principales.
PHI1 <- Y %*% M %*% MU1
...
INDIV<-cbind(PHI1,PHI2)
INDIV
#9.NORMALE Affichez avec la commande plot les 15 individus projetes sur 
# les 2 premiers axes factoriels. 
# Avec la commande text, rajoutez le nom de ces individus. 
# Avec la commande axis, creer les axes
# axis(1,-10:10,pos=0,labels=FALSE)
...
#10.NORMALE Interpretez les deux axes factoriels.
#pour une représentation NS-EO
INDIV<-cbind(PHI2,PHI1)
rownames(INDIV) <-rownindiv
plot(INDIV)
...
# 11.NORMALE On pose Ek = vect(u1; · · · ; uk)
# On rappelle que la qualit´e de repr´esentation de l'individu 
# yi sur Ek est  
#   QLT (yi; Ek) =sum(alpha = 1..k) [phi_i_alpha/||yi||_M]²
# Calculer pour tous les individus la qualite de representation
...
# 12.NORMALE On rappelle que la contribution relative de l'individu yi 
# a l'inertie de l'axe a est :
#   CTRa(yi) = pi (PHIi;a)²/lamba_a
# Calculer pour tous les individus la contribution relative 
# au premier axe.
...
T.pca<-PCA(T[,-c(13:17)], scale.unit = FALSE)
T.pca<-PCA(T[,-c(13:17)], scale.unit = TRUE)
...
T.pca <- PCA(T[,-c(13:17)])
T.pca <- PCA(T[,-c(13:17)], scale.unit = FALSE)
T.pca <- PCA(T[,-c(13:17)],ind.sup=16:18)
T.pca <- PCA(T[,-c(13:17)],scale.unit = FALSE,ind.sup=16:18)
...
rownames(T.pca$ind)
plot(EO,NS)
text (x=EO[1:35], y= NS[1:35], labels =rownames(T.pca$ind$coord), col = 1 , pos = 1)
